...
void terminal_putintat(int c, uint8_t color, size_t x, size_t y) {
    const size_t index = y * VGA_WIDTH + x;
    terminal_buffer[index] = vga_entry(c, color);
}
...
void terminal_putnum(int n) {
    terminal_putintat(n+48, terminal_color, terminal_column, terminal_row);
    if (++terminal_column == VGA_WIDTH) {
        terminal_column = 0;
        terminal_row++;
    }
...
void kernel_main(void) {
    terminal_initialize();
    terminal_writestring("Hello, kernel World!\nThis OS now supports newlines...\n...and it even scrolls!");
    
    // Fill the screen to test scrolling
    for(int i = 0; i < 30; i++) {
